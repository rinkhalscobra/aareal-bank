  ...
  return (
    <div className="bg-white p-8 shadow-lg">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h3 className="text-2xl font-bold text-[#172033]">{t.financialEducation.calculators.debtCalc.title}</h3>
      </div>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              className="w-full h-2 bg-[#DDE7E3] rounded-lg appearance-none cursor-pointer"
            />
            <p className="text-xs text-[#788696] mt-1">
              {t.financialEducation.calculators.debtCalc.minimumPayment}: ${(balance * 0.02).toFixed(0)}
            </p>
          </div>

          {result.warning && (
            <div className="bg-[#ECFDF8] p-4 rounded-lg border-l-4 border-[#C8942D]">
              <div className="flex items-start gap-3">
                <AlertCircle className="w-5 h-5 text-[#0F766E] flex-shrink-0 mt-0.5" />
                <div>
                  <p className="text-sm font-bold text-[#12355B] mb-1">{t.financialEducation.calculators.debtCalc.paymentTooLow}</p>
                  <p className="text-sm text-[#0B5F59]">
          ...
          <div className="bg-[#ECFDF8] p-4 rounded-lg border-l-4 border-[#0F766E]">
            <p className="text-sm font-bold text-[#172033] mb-2">{t.financialEducation.calculators.debtCalc.strategies}:</p>
          ...
          {!result.warning && (
            <>
              ...
              <div className="bg-[#F6F8F7] p-6 rounded-lg border-2 border-[#DDE7E3]">
                <h4 className="text-lg font-bold mb-4 text-[#172033]">{t.financialEducation.calculators.debtCalc.breakdown}</h4>
                <div className="space-y-3">
                  <div className="flex justify-between items-center pb-3 border-b border-[#DDE7E3]">
                    <span className="text-[#3F4D5F]">{t.financialEducation.calculators.debtCalc.originalBalance}</span>
                    <span className="font-bold text-[#172033]">${balance.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between items-center pb-3 border-b border-[#DDE7E3]">
                    <span className="text-[#3F4D5F]">{t.financialEducation.calculators.debtCalc.totalInterest}</span>
                    <span className="font-bold text-[#0F766E]">${result.totalInterest.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span>
                  </div>
                  <div className="flex justify-between items-center pt-2">
                    <span className="text-[#172033] font-bold">{t.financialEducation.calculators.debtCalc.totalPaid}</span>
                    <span className="text-2xl font-bold text-[#172033]">${result.totalPaid.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-[#F6F8F7] p-6 rounded-lg border-2 border-[#DDE7E3]">
                <h4 className="text-lg font-bold mb-3 text-[#172033]">{t.financialEducation.calculators.debtCalc.progressViz}</h4>
                <div className="space-y-3">
                  <div>
                    <div className="flex justify-between mb-2 text-sm">
                      <span className="text-[#3F4D5F]">{t.financialEducation.calculators.debtCalc.principal}</span>
                      <span className="font-semibold text-[#172033]">{((balance / result.totalPaid) * 100).toFixed(0)}%</span>
                    </div>
                    <div className="w-full bg-[#DDE7E3] rounded-full h-4">
                      <div
                        className="bg-[#0F766E] h-4 rounded-full"
                  ...
                  <div>
                    <div className="flex justify-between mb-2 text-sm">
                      <span className="text-[#3F4D5F]">{t.financialEducation.calculators.debtCalc.interest}</span>
                      <span className="font-semibold text-[#0F766E]">{((result.totalInterest / result.totalPaid) * 100).toFixed(0)}%</span>
                    </div>
                    <div className="w-full bg-[#DDE7E3] rounded-full h-4">
                      <div
                        className="bg-[#0F766E] h-4 rounded-full"
